...
parser.add_argument('-nv', '--noise_variance', type=int, default=0, help="noise variance level of the injected Gaussian Noise")
...
#parser.add_argument('-ha', '--hard_assign', type=str, default='8,2,2', help="hard assign number of roles in the network, order by worker, validator and miner. e.g. 12,5,3 assign 12 workers, 5 validators and 3 miners. \"*,*,*\" means completely random role-assigning in each communication round ")
...
if __name__=="__main__":
	...
	if args['resume_path']:
		...
		roles_requirement = config.roles_assign
		try:
		...
		with open(f'{log_files_folder_path}/args_used.txt', 'w') as f:
			...
			f.write(f"\n--poison learning rate: {config.poison_lr}")
			f.write(f"\n--decay: {config.decay}")
			f.write(f"\n--num of devices {config.num_devices}")
			f.write(f"\n--roles assigned: {config.roles_assign}")
			f.write(f"\n--num of malicious: {len(config.adversarial_list)}")
			f.write(f"\n--malicious devices: {config.adversarial_list}")
			f.write(f"\n--poison start round: {config.start_poison_round}")
			f.write(f"\n--detection start round: {config.start_detection_round}")
			f.write(f"\n--poison per batch: {config.poisoning_per_batch} / {config.batch_size}")
			f.write(f"\n--attacking shot: {config.attacking_shot}")
			f.write(f"\n--global update method: {config.global_update_method}")
			f.write(f"\n--roles assign method: {config.roles_assign_method}")
			f.write(f"\n--data assign: {config.data_assign}")
		...
		if args['save_network_snapshots']:
			...
			os.makedirs(network_snapshot_save_path)
		...
		num_malicious = len(config.adversarial_list)
		...
		open(f"{log_files_folder_path}/nodes.txt", 'w').close()
		open(f"{log_files_folder_path}/edges.txt", 'w').close()
	...
	for comm_round in range(latest_round_num + 1, args['max_num_comm']+1):
		...
		# calculate contribution of each device
		contribution = []
		total_contribution = 0
		for device in devices_list:
			print(f"{device.data_size}: {device.worker_epochs}\n")
			contribution.append(device.data_size * device.worker_epochs)
			total_contribution += device.data_size * device.worker_epochs
		p = [float(contribution[i] / total_contribution) for i in range(num_devices)]
		workers_this_round = np.random.choice(devices_list, size=workers_needed, replace=False, p=p)
		print(f"[DEBUG] Probability: {p}\n")

		for device in devices_list:
			if device in workers_this_round:
				...
				device.assign_role()
			if device.return_role() == 'miner':
				miners_this_round.append(device)
			elif device.return_role() == 'validator':
		...
		print(''' Logging Attack Success Rate ''')

		if config.global_update_method == 'all_devices': # only record data of one device
			device = devices_list[0]
			local = []
			for i in range(0, config.trigger_num):
				local.append(device.test_local_trigger(test_data, i))
			global_rate = device.test_global_trigger(test_data)
			with open(f"{log_files_folder_path_comm_round}/attack_comm_{comm_round}.txt", "a") as file:
				file.write(f"Global trigger: {global_rate}\n")
				for j in range(0, config.trigger_num):
					file.write(f"local trigger {j+1}: {local[j]}\n")

		elif config.global_update_method == 'only_workers': # record data of all devices
			global_trigger = {}
			for device in devices_list:
				local = []
				for i in range(0, config.trigger_num):
					local.append(device.test_local_trigger(test_data, i))
				global_rate = device.test_global_trigger(test_data)
				global_trigger[device.idx] = global_rate
				is_malicious_node = "M" if device.return_is_malicious() else "B"
				with open(f"{log_files_folder_path_comm_round}/{device.idx}_asr_comm_{comm_round}.txt", "a") as file:
					file.write(f"{device.return_idx()} {is_malicious_node}\n")
					file.write(f"role: {device.return_role()}\n")
					file.write(f"Global trigger: {global_rate}\n")
					for j in range(0, config.trigger_num):
						file.write(f"local trigger {j+1}: {local[j]}\n")
			
			# record nodes and edges of digraph
			'''for worker in workers_this_round:
				nodes_cnt += 1
				is_malicious = "M" if worker.return_is_malicious() else "B"
				asr_this_round = global_trigger[worker.idx]
				with open(f"{log_files_folder_path_comm_round}/nodes.txt", "a") as f:
					f.write(f"{nodes_cnt} {worker.idx.split('_')[-1]} {is_malicious} {comm_round} {asr_this_round}\n")'''