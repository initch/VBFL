...
class Device:
	def __init__(self, idx, assigned_train_ds, assigned_test_ds, local_batch_size, worker_epochs, learning_rate, loss_func, opti, network_stability, net, dev, miner_acception_wait_time, miner_accepted_transactions_size_limit, validator_threshold, pow_difficulty, even_link_speed_strength, base_data_transmission_speed, even_computation_power, is_malicious, noise_variance, check_signature, not_resync_chain, malicious_updates_discount, knock_out_rounds, lazy_worker_knock_out_rounds):
		...
		self.train_dl = DataLoader(self.train_ds, batch_size=config.batch_size, shuffle=True)
		self.test_ds = assigned_test_ds
		self.data_size = len(self.train_ds)
		...
		if self.is_malicious:
			self.opti = optim.Adam(self.net.parameters(), lr=config.poison_lr, weight_decay=config.decay)
		else:
			self.opti = optim.Adam(self.net.parameters(), lr=config.lr, weight_decay=config.decay)
			...
			if mined_by in self.black_list:
				...
				for worker_idx, local_updates_direction_records in valid_transactions_records_by_worker.items():
					if len(local_updates_direction_records['negative_epochs']) >  len(local_updates_direction_records['positive_epochs']):
						...
						for comm_round_to_check in range(comm_round):
				...
				elif config.global_update_method == 'only_workers':
					if self.online_switcher() and self.role == 'worker':
						self.global_update(finally_used_local_params)
					else:
						print(f"[Hint] {self.role} {self.idx} doesn't do global updates in this comm round.")
	...
	def worker_local_update(self, rewards, log_files_folder_path_comm_round, comm_round, local_epochs=1):
		...
		for epoch in range(local_epochs):
			...
			correct = 0
			total_loss = 0
			if self.is_malicious and comm_round >= config.start_poison_round and self.shot < config.attacking_shot:
				self.shot += 1
				poison_data_count = 0
				for data, label in self.train_dl:
					index = helper.get_adversarial_index(int(self.idx.split("_")[-1]) - 1)
					data, label, poison_num = helper.get_poison_batch(data, label, adversarial_index=index)
			...
			with open(f"{log_files_folder_path_comm_round}/worker_{self.idx}_{is_malicious_node}_local_updating_accuracies_comm_{comm_round}.txt", "a") as file:
				file.write(
					f"{self.return_idx()} epoch_{epoch+1} {self.return_role()} {is_malicious_node}, train_acc: {acc}, avg loss: {total_l}\n")
			self.local_total_epoch += 1
		# local update done
		try:
			self.local_update_time = (
				time.time() - self.local_update_time)/self.computation_power
		except:
	...
	def test_poison_trigger(self, test_ds, adv_index=-1, weights_to_eval=None):
		if weights_to_eval:
		...
		with torch.no_grad():
			...
			poison_data_count = 0
			dataset_size = 0
			for data, targets in poison_test_dl:
				data, targets, poison_num = helper.get_poison_batch(data, targets, adversarial_index=adv_index, evaluation=True)
	...
	def test_global_trigger(self, test_ds, weights_to_eval=None):
		acc, loss = self.test_poison_trigger(test_ds, weights_to_eval=weights_to_eval)
		return acc

	def test_local_trigger(self, test_ds, index, weights_to_eval=None):
		acc, loss = self.test_poison_trigger(test_ds, index, weights_to_eval=weights_to_eval)
		return acc
	...
	def waste_one_epoch_local_update_time(self, opti):
		if self.computation_power == 0:
			...
			for param_group in self.opti.param_groups:
				...
				validation_opti = optim.Adam(validation_net.parameters(), lr=currently_used_lr, weight_decay=config.decay)
	...
	def validate_model_weights(self, weights_to_eval=None):
		with torch.no_grad():
			...
			sum_accu = 0
			num = 0
			for inedx, batch in enumerate(self.test_dl):
				data, label = batch
				data, label = data.to(self.dev), label.to(self.dev)
				preds = self.net(data)
	...
	def validator_backdor_detection(self, weights_to_eval):
		masks = reverse_engineer(self.net, weights_to_eval, self.train_dl)
		flag_list = analyze_pattern_norm_dist(masks)
		return flag_list
...
#		self.block_to_add = None
	...
	def validator_update_model_by_one_epoch_and_validate_local_accuracy(self, opti):
		...
		if self.computation_power == 0:
			...
			for param_group in self.opti.param_groups:
				currently_used_lr = param_group['lr']
			# by default use Adam. Did not implement others
			validation_opti = optim.Adam(updated_net.parameters(), lr=currently_used_lr, weight_decay=config.decay)
	...
	def validate_worker_transaction(self, transaction_to_validate, rewards, log_files_folder_path, comm_round, malicious_validator_on):
		...
		if self.computation_power == 0:
			...
			if transaction_to_validate['worker_signature_valid']:
				...
				backdoor_flag = False
				if comm_round >= config.start_detection_round:
					flag_list =  self.validator_backdor_detection(transaction_to_validate["local_updates_params"])
					if flag_list:
						backdoor_flag = True
						with open(f"{log_files_folder_path_comm_round}/validator_{self.idx}_{is_malicious_validator}_validation_records_comm_{comm_round}.txt", "a") as file:
							file.write(f"{worker_transaction_device_idx} outlier label: {flag_list}\n")
				...
				if accuracy_by_worker_update_using_own_data - self.validator_local_accuracy < self.validator_threshold * -1 or backdoor_flag:
...
class DevicesInNetwork(object):
	def __init__(self, data_set_name, is_iid, batch_size, local_epochs, learning_rate, loss_func, opti, num_devices, network_stability, net, dev, knock_out_rounds, lazy_worker_knock_out_rounds, shard_test_data, miner_acception_wait_time, miner_accepted_transactions_size_limit, validator_threshold, pow_difficulty, even_link_speed_strength, base_data_transmission_speed, even_computation_power, malicious_updates_discount, num_malicious, noise_variance, check_signature, not_resync_chain):
	...
	# distribute the test dataset evenly to the devices
	def data_set_allocation(self):
		...
		# shard train
		shard_size_train = mnist_dataset.train_data_size // sum(config.data_assign)
		shards_id_train = np.random.permutation(config.num_devices)
		...
		for i in range(self.num_devices):
			...
			local_train_data = train_data[start_id_train * shard_size_train: start_id_train * shard_size_train + config.data_assign[i] * shard_size_train]
			local_train_label = train_label[start_id_train * shard_size_train: start_id_train * shard_size_train + config.data_assign[i] * shard_size_train]
			local_train_label = np.argmax(local_train_label, axis=1)

			start_id_train += config.data_assign[i]
